...
const TrekGallery = () => {
    ...
    useEffect (()=>{
        ...
        axios.get('http://192.168.1.23:4000/gallery/getAllTrek')
        .then((response)=>{
        //  console.log(response.data);
        //  console.log(response.data.people);
        //  console.log(response.data.people);
        
        setImages(response.data.trek);
        })
        .catch((error)=>{
         console.log(error);
        })
    ...
    const imgAction = (action) => {
        ...
        if(action ==='next-img'){
            if(i < images.length){
            setData({img: images[i].imgTrek, i: i});
           // console.log(images[i].imgPeople);
          // console.log(images.length);
            }
            else{
            setData({img: images[0].imgTrek, i: 0})
            }
         //  console.log(i);
        ...
        if(action === 'previous-img'){
            ...
            console.log(i);
            if(i === 1){
                setData({img: images[images.length - 1].imgTrek, i: images.length - 1});
         //  console.log(images.length);
        //    if(i === images.length ){
        //     setData({img: images[i-1].imgPeople, i: 4})
            }
            else{
                setData({img: images[j-1].imgTrek, i: j-1});
            }
...
<section className='trek-gallery-wrrapper'>
 ...
 { data.img &&
                   ...
                   ><FaWindowClose className='icon-close'/></button>
                    ...
                    ><BiSolidLeftArrow className='icon-control'/></button>
                      <img alt="tt" src={`http://192.168.1.23:4000/gallery/`+data.img} 
                   ...
                   <div className='gallery-inner-head-wrapper'>
                    <h5>Trek</h5><Link to='/gallery'><BiChevronLeft/>Album list</Link>
                </div>      
            ...
            >
                <Masonry gutter = "20px" >
                    {images.map((image, i) => (
                        <img
                            key={i}
                            src={`http://192.168.1.23:4000/gallery/`+image.imgTrek}
                            style={{ width: "350px", height: "250px", display: "block", cursor:'pointer' }}
                            alt="immmmg"
                            onClick ={() => viewImage(image.imgTrek, i)}
                        />
                    ))}
                </Masonry>
            </ResponsiveMasonry>
