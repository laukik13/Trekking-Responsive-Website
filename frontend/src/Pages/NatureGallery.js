...
const NatureGallery = () => {
    ...
    useEffect (()=>{
        ...
        setImages(response.data.nature);
        })
        .catch((error)=>{
         console.log(error);
        })
    ...
    const imgAction = (action) => {
        ...
        if(action ==='next-img'){
            if(i < images.length){
            setData({img: images[i].imgNature, i: i});
           // console.log(images[i].imgPeople);
          // console.log(images.length);
            }
            else{
            setData({img: images[0].imgNature, i: 0})
            }
         //  console.log(i);
        ...
        if(action === 'previous-img'){
            ...
            console.log(i);
            if(i === 1){
                setData({img: images[images.length - 1].imgNature, i: images.length - 1});
         //  console.log(images.length);
        //    if(i === images.length ){
        //     setData({img: images[i-1].imgPeople, i: 4})
            }
            else{
                setData({img: images[j-1].imgNature, i: j-1});
            }
...
<section className='nature-gallery-wrrapper'>
 ...
 { data.img &&
                   ...
                   <div className='gallery-inner-head-wrapper'>
                    <h5>Nature</h5><Link to='/gallery'><BiChevronLeft/>Album list</Link>
                </div>      
            ...
            >
                <Masonry gutter = "20px" >
                    {images.map((image, i) => (
                        <img
                            key={i}
                            src={`http://192.168.1.23:4000/gallery/`+image.imgNature}
                            style={{ width: "350px", height: "250px", display: "block", cursor:'pointer' }}
                            alt="immmmg"
                            onClick ={() => viewImage(image.imgNature, i)}
                        />
                    ))}
                </Masonry>
            </ResponsiveMasonry>
