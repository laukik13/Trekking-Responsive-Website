...
const OtherGallery = () => {
    ...
    const imgAction = (action) => {
        ...
        if(action ==='next-img'){
            if(i < images.length){
            setData({img: images[i].imgOther, i: i});
           // console.log(images[i].imgPeople);
          // console.log(images.length);
            }
            else{
            setData({img: images[0].imgOther, i: 0})
            }
         //  console.log(i);
        ...
        if(action === 'previous-img'){
            ...
            console.log(i);
            if(i === 1){
                setData({img: images[images.length - 1].imgOther, i: images.length - 1});
         //  console.log(images.length);
        //    if(i === images.length ){
        //     setData({img: images[i-1].imgPeople, i: 4})
            }
            else{
                setData({img: images[j-1].imgOther, i: j-1});
            }
...
<section className='other-gallery-wrrapper'>
 ...
 { data.img &&
                   ...
                   <div className='gallery-inner-head-wrapper'>
                    <h5>Other</h5><Link to='/gallery'><BiChevronLeft/>Album list</Link>
                </div>      
            ...
            >
                <Masonry gutter = "20px" >
                    {images.map((image, i) => (
                        <img
                            key={i}
                            src={`${process.env.REACT_APP_HOST_URL}/gallery/`+image.imgOther}
                            style={{ width: "350px", height: "250px", display: "block", cursor:'pointer' }}
                            alt="immmmg"
                            onClick ={() => viewImage(image.imgOther, i)}
                        />
                    ))}
                </Masonry>
            </ResponsiveMasonry>
